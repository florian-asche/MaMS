...
class JobClass {
    ...
    public function load_data($jobdata) {
        ...
        $temp = $ConnectClass->tcp_connect($ip, $port, "bmp085 temp" . "\r\n", $answerstrings, $number_lines, $eol_delimiters, $connect_debug);
        $output->debug(__FUNCTION__, "[RETURNED-DATA] " . $output->array_output($temp), "3");
        
        $apress = $ConnectClass->tcp_connect($ip, $port, "bmp085 apress" . "\r\n", $answerstrings, $number_lines, $eol_delimiters, $connect_debug);
        $output->debug(__FUNCTION__, "[RETURNED-DATA] " . $output->array_output($apress), "3");
        ...
        // Filter for apress
        if (preg_match("/([0-9.+-]+)/", $apress[0])) {
            $absolute_pressure = $apress[0] / 100;
            $output->debug(__FUNCTION__, "[DATA] Absolute Pressure: " . $absolute_pressure . " Pa", "3");
        } else {
            $output->debug(__FUNCTION__, "[ERROR] pattern match on temperature failed", "1");
            return 1;
        }
