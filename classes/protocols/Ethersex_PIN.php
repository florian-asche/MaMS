...
class JobClass {
    ...
    public function load_data($jobdata) {
        ...
        if (isset($jobdata['station_protocols']['configuration']['pin_channel'])) {
            $pin_channel = $jobdata['station_protocols']['configuration']['pin_channel'];
            $pin_channel = strtolower($pin_channel);
        } else {
            $output->debug(__FUNCTION__, "[ERROR] missing pin_channel in station_protocols configuration", "3");
            return 1;
        }
        ...
        $pin = $ConnectClass->tcp_connect($ip, $port, "pin get " . $pin_channel . "\n", $answerstrings, $number_lines, $eol_delimiters, $connect_debug);
        
        if (!isset($pin[0])) {
            $output->debug(__FUNCTION__, "[ERROR] pin raw failed", "1");
            return 1;
        }
    ...
    public function send_data($jobdata) {
        ...
        //$data = $ConnectClass->tcp_connect($ip, $port, $request, $answerstring, $number_lines, $eol_delimiters, $connect_debug);
        $set = $ConnectClass->tcp_connect($ip, $port, "pin set " . $pin_channel . " " . $pin_set . "\r\n", $answerstrings, $number_lines, $eol_delimiters, $connect_debug);
