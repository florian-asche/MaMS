            ...
            foreach ($response as $responseforeach) {                                           // Schleife für jeden Datensatz
                ...
                if (preg_match_all($RECEIVEFILTER, $responseforeach, $matches, PREG_SET_ORDER)) {  // der dieser Maske entspricht
                    ...
                    if ($luftfeuchtigkeit >= "255") {                                                                           // Falsche Temperaturwerte rausfiltern
                        $sql_neu = "INSERT INTO `data` (`wert1`, `wert2`, `objectid`,`timestamp`) VALUES ('$temperatur', '$luftfeuchtigkeit', '$MAMS_objektid', NOW(), '1')"; //Syntax für Datenbank
                    } else {                                                                                                    // Anfang Daten in die Datenbank Schreiben (Wenn der Wert ok ist)
                        $sql_neu = "INSERT INTO `data` (`wert1`, `wert2`, `objectid`,`timestamp`) VALUES ('$temperatur', '$luftfeuchtigkeit', '$MAMS_objektid', NOW())"; //Syntax für Datenbank
                    };
