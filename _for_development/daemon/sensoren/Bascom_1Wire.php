...
function funktion_station_getdata($IP,$PORT,$SENDCOMMAND,$RECEIVEFILTER,$RECIEVECOMMAND,$MAMS_objektid,$absolut_pfad,$link,$defaultoutput,$PARAMETER1,$PARAMETER2,$PARAMETER3) {
    if (!$SENDCOMMAND) {
        $SENDCOMMAND = "1wire";                                                 // Default SENDCOMMAND if no other given
        ...
        if ($debug == "0") {                                                    // Prüfen ob Debug abgeschaltet (Damit er keinen muell in die DB schreibt)
            foreach ($response as $responseforeach) {                           // Schleife für jeden Datensatz
                if (preg_match_all($RECEIVEFILTER, $responseforeach, $matches, PREG_SET_ORDER)) {       // der dieser Maske entspricht
                    ...
                    $objektid = $matches[0][1];                                                 // Gefundene objektid in Variable schreiben
                    $temperatur = $matches[0][3];                                               // Gefundene Temperatur in Variable schreiben

                    if ($defaultoutput == "1") { 
                        echo "Sensor <b>" . $objektid . "</b> gefunden..." . "<br>" . "Die Temperatur betraegt <b>" . $temperatur . " Grad</b>"; // Debug Ausgabe
                    }; 

                    if ($temperatur >= "85") {                                                  // Falsche Temperaturwerte rausfiltern
                        $sql_neu = "INSERT INTO `data` (`wert1`, `objectid`,`timestamp`) VALUES ('" . $temperatur . "', '" . $objektid . "', NOW(), '1')"; //Syntax für Datenbank
                    } else {                                                                    // Anfang Daten in die Datenbank Schreiben (Wenn der Wert ok ist)
                        $sql_neu = "INSERT INTO `data` (`wert1`, `objectid`,`timestamp`) VALUES ('" . $temperatur . "', '" . $objektid . "', NOW())"; //Syntax für Datenbank
                    };
