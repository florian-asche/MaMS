...
//$debug = "1";
//
//$IP   = '[caca::50:c4ff:fe04:8168]';
//$PORT = 61616;
//$funktion = "get";
//
//if ($funktion == "get") {
//    echo $rueckgabewert = request($IP,$PORT,"getstatus",$debug);
//    echo "<br>";
//    echo $rueckgabewert = request($IP,$PORT,"getpower",$debug);
//} elseif ($funktion == "on") {
//    $rueckgabewert = request($IP,$PORT,"seton",$debug);
//    $rueckgabewert2 = request($IP,$PORT,"getstatus",$debug);
//} elseif ($funktion == "off") {
//    $rueckgabewert = request($IP,$PORT,"setoff",$debug);
//    $rueckgabewert2 = request($IP,$PORT,"getstatus",$debug);
//} else {
...
//}
//
//if ($rueckgabewert != "ERROR") {
//    echo $rueckgabewert;
//};
//
//if ($rueckgabewert2 != "ERROR") {
//    echo $rueckgabewert2;
//};


function udpconnector_hexabus_ausgabe($packetType,$header_hex,$packetType_hex,$flags_hex,$ErrorCode_hex,$eid_hex,$data_type_hex,$value_hex,$crc1_hex,$crc2_hex,$header,$flags,$ErrorCode,$eid,$data_type,$value,$crc1,$crc2) {
...
function str2hex_encoder1($string) {
    echo $string;
    echo "---";
...
function request($IP,$PORT,$funktion,$debug,$debug_array) {
    $debug_array = "1";
    $timeout = "20";

    if ($funktion == "getpower") {
        $request     = chr(0x48).chr(0x58).chr(0x30).chr(0x43).chr(0x02).chr(0x00).chr(0x00).chr(0x00).chr(0x00).chr(0x02).chr(0xf7).chr(0xcb);                       //GET POWER
        $getdata = true;
    } elseif ($funktion == "getstatus") {
        $request     = chr(0x48).chr(0x58).chr(0x30).chr(0x43).chr(0x02).chr(0x00).chr(0x00).chr(0x00).chr(0x00).chr(0x01).chr(0xc5).chr(0x50);                       //GET STATUS
        $getdata = true;
    } elseif ($funktion == "seton") {
        $request     = chr(0x48).chr(0x58).chr(0x30).chr(0x43).chr(0x04).chr(0x00).chr(0x00).chr(0x00).chr(0x00).chr(0x01).chr(0x01).chr(0x01).chr(0x57).chr(0xb6);   //ON
    } elseif ($funktion == "setoff") {
        $request     = chr(0x48).chr(0x58).chr(0x30).chr(0x43).chr(0x04).chr(0x00).chr(0x00).chr(0x00).chr(0x00).chr(0x01).chr(0x01).chr(0x00).chr(0x46).chr(0x3f);   //OFF
    } else {
        echo "ERROR: unbekannte Funktion!";
        return "ERROR";
    }
    ...
    if (!$fp) {
        ...
        $write = fwrite($fp, $request);
        if (!$write) {
            echo "CONNECTION ERROR: cannot writing to Port!";
            return "ERROR";
        } else {
            if (isset($getdata)) {
                ...
                $returndata_hex = str2hex_encoder1($returndata);
                ...
                if ($header_hex == "0x480x580x300x43" or $header_hex == "0x480x580x300x42") {   // Header pruefen
                    ...
                    // DEBUG AUSGABE
                    if ($debug == "1") {
                        udpconnector_hexabus_ausgabe($packetType,$header_hex,$packetType_hex,$flags_hex,$ErrorCode_hex,$eid_hex,$data_type_hex,$value_hex,$crc1_hex,$crc2_hex,$header,$flags,$ErrorCode,$eid,$data_type,$value,$crc1,$crc2);
                    };

                } else {
                    echo "Header NICHT OK (" . $header . ")";
                    return "ERROR";
                };
